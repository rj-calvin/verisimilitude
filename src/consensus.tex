\startcomponent consensus

\chapter{Consensus}

In the study of communication, the Byzantine generals problem, as formulated by
Leslie Lamport, is foundational. The impossibility theorem that it inspires
brings madness to all who understand its scope. However, to the optimist,
impossibility is the greatest of news since knowledge of what is impossible
improves the efficiency of our reason.

Before discourse on the general problem of the Byzantine generals can begin, it
is first necessary to discuss the simpler problem of only two generals. This
simpler excersice will be used to inspire a formal definition of
\emph{consensus} which will be used in later discussion. The formal analogy of
the two generals is stated as such:
...
general informed of this alarm could reason that they no longer need to send
messengers to their peer in order to coordinate an attack. Instead, if the
...
formal language. The practice of teleological deduction, then, seems to demand
of us a certain humility. An admission to the incompleteness of any language
invented by the mind that is destined for the interpretation of others.

If it is our intention to formalize consensus between the two generals, it is
necessary that we construct a protocol that allows the two generals to use a
finite number of messengers to maximize the simultaneity of their attack. Since
this is a new interpretation of the original two generals problem, we must
first reframe our desired outcome using a new formal analogy:

\startframedtext
Two generals find themselves on opposite sides of enemy territory while they
wait for reinforcements. Each general knows the position of the other, but not
their movements, behavior, or how many reinforcements they have received; each
knows that their only means of communication is via messengers that must travel
through treacherous territory; and each knows how to distinguish enemy from
ally. In addition, the generals are also both aware that the enemy is equipped
with a horn that will blow whenever an attack begins. The question to be
answered is whether it is possible for the generals to coordinate an attack
only when both allied armies have gathered sufficient reinforcements.
\stopframedtext

The key to the construction is to define how to utilize a game of coinage to
communicate the rate of reinforcement of one general's army. This rate of
change is not a measurement in the classical sense, but rather, it is an
indicator of confidence based on the size of the sending general's army
relative to the enemy. The next step will then concretely define the
interpretation of a message and show how this can be used to improve the rigor
of what is understood as confidence.
